...
class Agent(nn.Module):
    def __init__(self, mapsize=16*16):
        ...
        print("nvec[1:].sum:", envs.action_space.nvec[1:].sum())
        print("Actor nn out dim:", self.mapsize * envs.action_space.nvec[1:].sum())
    ...
    def get_action(self, x, action=None, invalid_action_masks=None, envs=None):
        ...
        grid_logits = logits.view(-1, envs.action_space.nvec[1:].sum())    #[hw * ambientes, 78]
        #print("grid_logits:\n", grid_logits[0])
        split_logits = torch.split(grid_logits, envs.action_space.nvec[1:].tolist(), dim=1) #([hw*am,6], [hw*am, 4], ....]
        #for x in split_logits:
            #print("split_logits:", x.size())
        #print("-----")

        if action is None:
            invalid_action_masks = torch.tensor(np.array(envs.vec_client.getMasks(0))).to(device)
            invalid_action_masks = invalid_action_masks.view(-1,invalid_action_masks.shape[-1])
            print("Action mask shape:", invalid_action_masks.size())
...
print("Grad critic:", agent.critic.weight.grad)
if args.anneal_lr:
...
mapsize = 16*16
actionespace_shape = (mapsize, envs.action_space.shape[0] - 1)
invalid_action_shape = (mapsize, envs.action_space.nvec[1:].sum()+1)

print("num_steps:", args.num_steps,"\nnum_envs:", args.num_envs)
print("observation space shape:", envs.observation_space.shape)
obs = torch.zeros((args.num_steps, args.num_envs) + envs.observation_space.shape).to(device)

print("obs buffer size:", obs.size())
...
print("action_masks size:", invalid_action_masks.size())
...
for update in range(starting_update, num_updates+1):
    ...
    for step in range(0, args.num_steps):
        ...
        # AQUI HAY QUE HACER EL TD 
        if step > 0:
            deltas = rewards[step - 1] + args.gamma * values[step] - values[step - 1]

            critic_loss = (deltas*deltas).mean()
            actor_loss = (deltas * logprobs[step]).mean()
